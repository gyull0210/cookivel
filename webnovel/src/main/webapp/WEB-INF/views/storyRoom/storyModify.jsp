...
<nav class="navbar navbar-expand navbar-light">
  <div class="container">
    <a class="navbar-brand fw-bold" href="#">작품 수정</a>
					...
					<div class="row justify-content-center">
							<div class="row mb-3">
								...
								<c:if test="${book.book_imgUrl != null}">
      								<div id="bookCoverBox" class="rounded" style="width:150px;height:210px; overflow:hidden;" data-bs-toggle="modal" data-bs-target="#coverUploader">
										<img id="bookCover" class="img-fluid" src="${bookCoverPath}">
									</div>
								</c:if>
								<c:if test="${book.book_imgUrl == null}">
      								<div id="bookCoverBox" class="rounded" style="width:150px;height:210px; overflow:hidden;" data-bs-toggle="modal" data-bs-target="#coverUploader">
										<img id="bookCover" class="img-fluid" src="http://www.placehold.it/150X180">
									</div>
								</c:if>
    							</div>
  							</div>
  							<div class="row mb-3">
    							...
    							<div class="col-sm-10">
      							<input type="text" class="form-control" id="bookTitle" name="book_title" value="${book.book_title}">
    							</div>
  							</div>
  							<div class="row mb-3">
    							<label for="bookIntro" class="col-sm-2 col-form-label">소개</label>
    							<div class="col-sm-10">
      							<textarea class="form-control" id="bookIntro" name="book_intro">${book.book_intro}</textarea>
    							</div>
  							</div>
    						...
    						<div class="col-sm-10">
      							<div class="form-check">
        							<input class="form-check-input" type="radio" name="book_writeType" id="bookWriteType1" value="free" <c:if test="${book.book_writeType eq 'free'}">checked</c:if>>
        							<label class="form-check-label" for="gridRadios1">
          							자유연재
        							</label>
      							</div>
      							<div class="form-check">
        							<input class="form-check-input" type="radio" name="book_writeType" id="bookWriteType2" value="premium" <c:if test="${book.book_writeType eq 'premium'}">checked</c:if>>
  							...
  							<div class="row mb-3">
    							...
    							<div class="col-sm-10">
      							<select class="form-select" id="bookGenre" name="book_genre">
      								<option value="" <c:if test="${book.book_genre eq ''}">selected</c:if>>장르선택</option>
      								<option value="판타지" <c:if test="${book.book_genre eq '판타지'}">selected</c:if>>판타지</option>
      								<option value="현대판타지" <c:if test="${book.book_genre eq '현대판타지'}">selected</c:if>>현대판타지</option>
      								<option value="무협" <c:if test="${book.book_genre eq '무협'}">selected</c:if>>무협</option>
      								<option value="로맨스" <c:if test="${book.book_genre eq '로맨스'}">selected</c:if>>로맨스</option>
      								<option value="로맨스판타지" <c:if test="${book.book_genre eq '로맨스판타지'}">selected</c:if>>로맨스판타지</option>
      								<option value="BL" <c:if test="${book.book_genre eq 'BL'}">selected</c:if>>BL</option>
      								<option value="GL" <c:if test="${book.book_genre eq 'GL'}">selected</c:if>>GL</option>
      							</select>
    							</div>
  							</div>
  							<div class="row mb-3">
    							...
    							<div class="col-sm-10">
      							<input type="text" class="form-control" id="bookTag">
    							</div>
  							</div>
  							<fieldset class="row mb-3">
    						...
    						<div class="col-sm-10">
      							<div class="form-check">
        							<input class="form-check-input" type="radio" name="book_state" id="bookState1" value="연재중" <c:if test="${book.book_state eq '연재중'}">checked</c:if>>
        							<label class="form-check-label" for="bookState1">
          							연재중
        							</label>
      							</div>
      							<div class="form-check">
        							<input class="form-check-input" type="radio" name="book_state" id="bookState2" value="완결됨" <c:if test="${book.book_state eq '완결됨'}">checked</c:if>>
  							...
  							<div class="row mb-3">
    							...
    							<div class="col-sm-10">
      								<div class="form-check">
        								<input class="form-check-input" type="checkbox" id="isHidden" name="book_isHidden" value="${book.book_isHidden}" <c:if test="${book.book_isHidden eq 'Y'}">checked</c:if>>
  							...
  							<div class="d-flex justify-content-between">
  								<button id="cancel" type="button" class="btn btn-outline-secondary" onclick="history.go(-1);">취소</button>
  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        ...
        <button id="imgNextBtn" type="button" class="btn btn-primary" data-bs-target="#coverCropper" data-bs-toggle="modal">다음</button>
      </div>
    </div>
  </div>
</div>
...
$(function(){
        ...
        if(imgFile.match(fileForm)) {
            ...
            $('#uploadBtn').on('click', function(){
        		...
        		canvas.toBlob(function (blob) {
            		...
            		formData.append('file', blob, $('#imgBtn').val().split('\\')[$('#imgBtn').val().split('\\').length-1].split('.')[0]+".png");   		
    ...
    $("#create").on("click", function(){   	
		...
		if(bookTitle ==""){
    		...
    		alert("제목은 2글자 이상, 30글자 미만으로 입력해주세요.");
    		return false;
    		...
    		formData.append('book_title', bookTitle.val());
    		formData.append('book_intro', bookIntro.val());
    		formData.append('book_genre', bookGenre.val());
			formData.append('book_writeType', bookWriteType.val());
            formData.append('book_state', bookState.val());
            formData.append('book_isHidden', bookIsHidden.val());
            
     		$.ajax('/storyRoom/storyModify', {
           		method: 'POST',
                data: formData,
                processData: false,
                contentType: false,
